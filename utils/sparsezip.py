...
class LayerwiseHybridScorer(nn.Module):
    def __init__(
        ...
        self.eps = eps
        self.skip_hybrid_attn = skip_hybrid_attn

        if use_layer_gating and num_layers > 1 and not skip_hybrid_attn:
    ...
    def _single_layer_hybrid(self, attn_weights: torch.Tensor, keys: torch.Tensor) -> torch.Tensor:
        ...
        # --- Ablation Branch: VisionZip Original Scoring ---
        if self.skip_hybrid_attn:
            # VisionZip Native: sum of attention heads from CLS to patches
            # cls_attention: [B, H, L-1] -> sum dim 1 -> [B, L-1]
            cls_attention = attn_weights[:, :, 0, 1:]
            return cls_attention.sum(dim=1) 
        
        # --- Normal SparseZip Logic ---
    ...
    def forward(
        ...
        # Cross Attention only applied if NOT skipping hybrid
...
class VisionZipCompressor(nn.Module):
    def __init__(self, num_scoring_layers: int = 1, cfg: CompressionConfig = CompressionConfig()) -> None:
        ...
        self.scorer = LayerwiseHybridScorer(
            ...
            tau_sim=cfg.tau_sim,
            skip_hybrid_attn=cfg.skip_hybrid_attn # 传递给 scorer
        )
    ...
    def forward(
        ...
        # 1. Scoring (Handled internally by updated LayerwiseHybridScorer)
        scores = self.scorer(scoring_layers, cross_attn=cross_attn, cross_last_dim_is_L=cross_last_dim_is_L)
        
        # 2. Selection (Dynamic K vs Fixed K)
        if self.cfg.dynamic_k and not self.cfg.skip_dynamic_k and dominant_num is None:
        ...
        for b in range(B):
            ...
            # --- Ablation Branch: VisionZip Original Merging ---
            if self.cfg.skip_ctx_merge:
                metric = scoring_layers[0]['keys'][b, mask, :] # [Nr, Ck] (using keys as metric)
                
                # VisionZip Native Logic Logic Implementation:
                # 1. Normalize Metric
                metric_normalized = F.normalize(metric.float(), dim=-1, eps=1e-12)
                
                contextual_num = self.cfg.merging.contextual_num
                Nr = metric_normalized.shape[0]
                
                if Nr > 0 and contextual_num > 0:
                    # 2. Uniform selection of targets
                    step = max(1, Nr // contextual_num)
                    target_indices = torch.arange(0, Nr, step, device=metric.device)[:contextual_num]
                    
                    # Handle padding if target_indices < contextual_num
                    if target_indices.shape[0] < contextual_num:
                        pad = torch.full((contextual_num - target_indices.shape[0],), target_indices[-1] if target_indices.numel()>0 else 0, device=target_indices.device, dtype=target_indices.dtype)
                        target_indices = torch.cat([target_indices, pad], dim=0)

                    target_tokens = metric_normalized[target_indices, :]
                    target_hidden = remain_hidden[target_indices, :]

                    # 3. Find tokens to merge (those NOT in targets)
                    # Note: VisionZip implementation uses logic that implicitly assigns every token to a target
                    # Here we implement the strict "Merge non-targets to targets" logic
                    all_indices = torch.arange(Nr, device=metric.device)
                    is_target = torch.isin(all_indices, target_indices)
                    
                    tokens_to_merge = metric_normalized[~is_target, :]
                    hidden_to_merge = remain_hidden[~is_target, :]
                    
                    if tokens_to_merge.shape[0] > 0:
                        # Similarity assignment
                        similarity = torch.mm(tokens_to_merge, target_tokens.t())
                        assign_idx = similarity.argmax(dim=1)
                        
                        # Aggregate (Scatter add)
                        assign_one_hot = torch.zeros(tokens_to_merge.shape[0], contextual_num, device=metric.device, dtype=metric.dtype)
                        assign_one_hot.scatter_(1, assign_idx.unsqueeze(1), 1.0)
                        
                        counts = assign_one_hot.sum(dim=0).clamp(min=1).unsqueeze(-1) # [Ctx, 1]
                        aggregated_hidden = torch.mm(assign_one_hot.t(), hidden_to_merge) / counts
                        
                        # VisionZip: target + aggregated
                        ctx_tokens = target_hidden + aggregated_hidden
                    else:
                        ctx_tokens = target_hidden
                else:
                    # Fallback if no remaining tokens
                    ctx_tokens = torch.zeros((0, C), device=hidden_states.device, dtype=hidden_states.dtype)

            else:
                # --- Normal SparseZip Logic (Hierarchical / Single-Shot) ---
