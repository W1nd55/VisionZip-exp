...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_sparse_inputs_labels_for_multimodal(
        ...
        # Get device from model parameters (handle multi-device case)
        try:
            model_device = next(self.parameters()).device
            if str(model_device) == 'meta':
                # Try to get from embed_tokens
                if hasattr(self.get_model(), 'embed_tokens'):
                    model_device = next(self.get_model().embed_tokens.parameters()).device
                else:
                    model_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        except (StopIteration, AttributeError):
            model_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
        # Move input_ids to model device if needed
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # Ensure all input_ids are on the same device as the embedding layer
            embed_device = next(self.get_model().embed_tokens.parameters()).device
            cur_input_ids_noim = [x.to(embed_device) for x in cur_input_ids_noim]
            cur_input_embeds = self.get_model().embed_tokens(torch.cat(cur_input_ids_noim)) 
            cur_input_embeds_no_im = torch.split(cur_input_embeds, split_sizes, dim=0) 
            cur_new_input_embeds = [] 
            cur_new_labels = []   
