...
# Patch transformers version check to allow newer tokenizers
import sys
import importlib
if 'transformers' in sys.modules:
    # If transformers is already imported, we need to patch it
    transformers_mod = sys.modules['transformers']
    if hasattr(transformers_mod, 'dependency_versions_check'):
        # Monkey-patch the version check to skip tokenizers check
        original_require = getattr(transformers_mod.dependency_versions_check, 'require_version_core', None)
        if original_require:
            def patched_require_version_core(req, hint=None):
                # Skip tokenizers version check
                if 'tokenizers' in req.lower():
                    return
                return original_require(req, hint)
            transformers_mod.dependency_versions_check.require_version_core = patched_require_version_core
...
# Try to import optional transformers functions/classes (may not exist in different versions)
try:
    from transformers.models.llama.modeling_llama import _prepare_4d_causal_attention_mask
except ImportError:
    # If _prepare_4d_causal_attention_mask doesn't exist, create a simple fallback
    def _prepare_4d_causal_attention_mask(attention_mask, input_shape, inputs_embeds, past_key_values_length):
        # Simple fallback - just return the attention mask as-is
        return attention_mask

try:
    from transformers.models.llama.modeling_llama import LlamaFlashAttention2
except ImportError:
    # If LlamaFlashAttention2 doesn't exist, use LlamaAttention as fallback
    LlamaFlashAttention2 = LlamaAttention

try:
    from transformers.models.llama.modeling_llama import LlamaSdpaAttention
except ImportError:
    # If LlamaSdpaAttention doesn't exist, use LlamaAttention as fallback
...
try:
    ...
    # If _prepare_4d_causal_attention_mask_for_sdpa doesn't exist, use regular one
    ...
    def forward(
        ...
        if (len(pre_prompt_length_list) != 0 and hidden_states.shape[1] !=1):
            ...
            if torch.cuda.is_available():
                ...
                # Dummy events for CPU
                class DummyEvent:
                    def record(self): pass
                    def synchronize(self): pass
                    def elapsed_time(self, other): return 0.0
                total_start_event = DummyEvent()
                total_end_event = DummyEvent()
                total_start_event.record()
...
class LlamaDynamicvitFlashAttention2(LlamaFlashAttention2):
    def _cpu_attention_forward(self, query_states, key_states, value_states, attention_mask, q_len, kv_seq_len, dropout_rate):
        """CPU-compatible attention computation fallback"""
        # query_states, key_states, value_states are in (bsz, seq_len, num_heads, head_dim) format
        # Reshape to (bsz, num_heads, seq_len, head_dim) for attention computation
        bsz = query_states.shape[0]
        query_states_fa = query_states.transpose(1, 2)  # (bsz, num_heads, seq_len, head_dim)
        ...
        attn_weights = torch.matmul(query_states_fa, key_states_fa.transpose(-2, -1)) / math.sqrt(self.head_dim)
        
        # Apply causal mask if needed
        if self.is_causal and attention_mask is None:
            causal_mask = torch.triu(torch.ones(q_len, kv_seq_len, dtype=torch.bool, device=query_states.device), diagonal=1)
            attn_weights.masked_fill_(causal_mask, float("-inf"))
        
        # Apply attention mask if provided
        if attention_mask is not None:
            if attention_mask.dim() == 4:  # (bsz, 1, q_len, kv_seq_len)
                attn_weights = attn_weights + attention_mask
            elif attention_mask.dim() == 2:  # (bsz, seq_len)
                attn_weights = attn_weights + attention_mask.unsqueeze(1).unsqueeze(1)
        
        # Softmax and dropout
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_weights = nn.functional.dropout(attn_weights, p=dropout_rate, training=self.training)
        
        # Apply attention to values: (bsz, num_heads, seq_len, head_dim)
        attn_output = torch.matmul(attn_weights, value_states_fa)
        # Reshape back to (bsz, seq_len, num_heads, head_dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(position_ids[0]) == 1:
            ...
            position_ids = torch.tensor([[past_key_value.get_usable_length(kv_seq_len, self.layer_idx)]], dtype=torch.int64).to(device)
        if past_key_value is not None:
            kv_seq_len += past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
        cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)
        ...
        # Use CPU-compatible attention if CUDA not available or flash attention not available
        if torch.cuda.is_available():
            try:
                ...
                # Flash attention not available (e.g., flash_attn_func not imported), fall back to regular attention
                attn_output = self._cpu_attention_forward(
                    query_states, key_states, value_states, attention_mask, q_len, kv_seq_len, dropout_rate
                )
        else:
            attn_output = self._cpu_attention_forward(
                query_states, key_states, value_states, attention_mask, q_len, kv_seq_len, dropout_rate
...
class LlamaDynamicvitDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # MODIFICATION: Check if flash_attn exists. If not, fall back to self_attn.
        if hasattr(self, 'flash_attn'):
            ...
            hidden_states, self_attn_weights, present_key_value, attn_logits = outputs[0], outputs[1], outputs[2], outputs[3]
            # Create dummy values for the ones flash_attn would have returned
            if sparse_layer:
                relation_vis_text = attn_logits
            else:
                attn_logits = None
        ...
        **model_kwargs,
        ...
        if torch.cuda.is_available():
            ...
            class DummyEvent:
                ...
                def synchronize(self): pass
                def elapsed_time(self, other): return 0.0
            causal_inference_start_event = DummyEvent()
            causal_inference_end_event = DummyEvent()
            causal_inference_start_event.record()
        while True:
            if synced_gpus:
        ...
        if torch.cuda.is_available():
            ...
            causal_inference_end_event.synchronize()
