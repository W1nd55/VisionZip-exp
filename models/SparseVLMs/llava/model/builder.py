        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if 'mpt' in model_name.lower():
                ...
                if dynamic_sparse:
                    ...
                    # DISABLED FOR MEMORY: The following loop adds new flash attention modules
                    # after the model is already allocated, causing OOM with device_map="auto".
                    # The core token pruning logic will still work without this optimization.
                    # for i in range(32):
                    #     # Get the device of this specific layer
                    #     try:
                    #         layer_device = next(model.model.layers[i].parameters()).device
                    #         if str(layer_device) == 'meta' or str(layer_device) == 'cpu':
                    #             # If layer is on CPU or meta, use CPU for flash attention
                    #             layer_device = torch.device('cpu')
                    #         eval_logger.info(f"Layer {i} flash attention -> {layer_device}")
                    #     except (StopIteration, AttributeError):
                    #         # Fallback to cuda if we can't detect
                    #         layer_device = torch.device(device if torch.cuda.is_available() else 'cpu')
                    #     
                    #     # Create flash attention on the same device as this layer
                    #     flash_attn = LlamaDynamicvitFlashAttention2(config=model.config, layer_idx=i).half().to(layer_device)
