    ...
    def __init__(self, model_path: str, dominant: int=54, contextual: int=10, temperature: float=0.2, max_new_tokens: int=256, sparsezip_cfg: Optional[dict] = None):
        ...
        try:
            ...
            def _sparsezip_forward(self, images):
                ...
                # Lazily create and cache compressor with contextual target
                if not hasattr(self, "_vz_comp"):
                    # Inherit configuration from YAML sparsezip_cfg captured on parent LlavaVisionZipModel
                    _cfg = getattr(self, "_sparsezip_cfg", {}) or {}
                    ...
                    cfg = CompressionConfig(
                        ...
                        merging=MergingConfig(
                            contextual_num=int(((_cfg.get("merging", {}) or {}).get("contextual_num", contextual_num))),
                    ...
                    self._vz_comp = VisionZipCompressor(num_scoring_layers=1, cfg=cfg)
                else:
                    # Update contextual target if it changed
                    try:
        ...
        try:
            ...
            if vt is not None and isinstance(sparsezip_cfg, dict):
