...
#         temperature: float = 0.2, 
#         max_new_tokens: int = 256, 
#         sparsezip_cfg: Optional[dict] = None
#     ):
#         # Imports necessary for model loading and processing
#         from llava.model.builder import load_pretrained_model
#         from llava.mm_utils import get_model_name_from_path
...
#             def _sparsezip_forward(self, images):
#                 """
#                 Replacement forward for CLIPVisionTower implementing SparseZip compression with Ablation Support.
#                 """
#                 # If list of images, keep baseline behavior: feature_select per image (no compression)
#                 if isinstance(images, list):
#                     image_features = []
...
#                 attn_weights = outs.attentions[-2]       # [B, H, L, L]
#                 hidden_states = outs.hidden_states[-2]   # [B, L, C]
#                 # Keys captured by prior monkey-patching
#                 keys = self.vision_tower.vision_model.encoder.layers[-2].metric  # [B, L, Ck]

#                 # Read contextual/dominant count from info
#                 contextual_num = 16
#                 dominant_num = 54
#                 try:
#                     contextual_num = int(self.vision_tower._info.get("contextual", 16))
#                     dominant_num = int(self.vision_tower._info.get("dominant", 54))
#                 except Exception:
#                     pass

#                 # Update contextual target dynamically if needed
#                 if hasattr(self, "_vz_comp"):
#                     try:
#                         self._vz_comp.cfg.merging.contextual_num = contextual_num
#                     except Exception:
...
#                 # --- Ablation Logic for Dynamic K ---
#                 # Default: pass None to let compressor handle dynamic K calculation
#                 pass_dominant = None
                
#                 # Check if 'skip_dynamic_k' is enabled in the attached compressor config
#                 if hasattr(self, "_vz_comp") and self._vz_comp.cfg.skip_dynamic_k:
#                     # If skipping dynamic K (ablation), force use of fixed dominant_num
#                     pass_dominant = dominant_num
...
#                     cross_attn=None,
#                     cross_last_dim_is_L=True,
#                     dominant_num=pass_dominant, # Passed explicit K if skipping dynamic logic
#                     weights_for_context=weights_for_context,
#                 )
...
#         # Simplified loading logic for brevity (same as your original)
#         quant_attempts = [
#             ({"load_4bit": True, "bnb_4bit_compute_dtype": torch.float16, "bnb_4bit_quant_type": "nf4"}, "4bit"),
#             ({"load_8bit": True}, "8bit"),
#             ({}, "fp16"),
#         ] if not is_macos else [({}, "fp16"), ({"torch_dtype": torch.float32}, "fp32")]
...
#         # ---- Construct VisionZipCompressor with Ablation Flags ----
#         try:
#             vt = getattr(model, "mm_projector", None)
#             vt = getattr(vt, "vision_tower", None)
#             if vt is not None:
...
#                     # --- Extract Ablation Switches ---
#                     _skip_hybrid = bool(_cfg.get("skip_hybrid_attn", False))
#                     _skip_dyn_k = bool(_cfg.get("skip_dynamic_k", False))
#                     _skip_merge = bool(_cfg.get("skip_ctx_merge", False))
...
#                             agglomerative=bool(((_cfg.get("merging", {}) or {}).get("agglomerative", True))),
#                         ),
#                         # --- Pass Ablation Flags to Config ---
#                         skip_hybrid_attn=_skip_hybrid,
#                         skip_dynamic_k=_skip_dyn_k,
#                         skip_ctx_merge=_skip_merge,
#                     )
                    
#                     print(f"[SparseZip] Init Compressor: skip_hybrid={_skip_hybrid}, skip_dynK={_skip_dyn_k}, skip_merge={_skip_merge}")
#                     setattr(vt, "_vz_comp", VisionZipCompressor(num_scoring_layers=1, cfg=cfg))
#                 except Exception as e:
#                     print("[Warn] Could not construct VisionZipCompressor in __init__:", e)
#         except Exception:
...
#         """Encodes an image through patched CLIP vision tower."""
...
#             try:
#                 c_ctx = vt._vz_comp.cfg.merging.contextual_num
#             except Exception:
#                 c_ctx = -1
#             stats = {
#                 'k_dominant': k_dom,
#                 'c_contextual': c_ctx,
#                 'orig_tokens': toks.shape[1],
...
#             from llava.mm_utils import tokenizer_image_token as _tik
#             def _wrap(prompt: str) -> torch.Tensor:
#                 return _tik(prompt, self.tokenizer, self.IMAGE_TOKEN_INDEX, return_tensors="pt")
#             return _wrap
#         except Exception:
#             print("[Warn] Could not import tokenizer_image_token, using local fallback.")
#             tok = self.tokenizer
...
#                 new_ids = []
#                 for t in ids:
#                     new_ids.append(self.IMAGE_TOKEN_INDEX if t == tok.convert_tokens_to_ids(self.DEFAULT_IMAGE_TOKEN) else t)
#                 return torch.tensor(new_ids, dtype=torch.long)
#             return _local
...
#     from scripts.timer import StageTimer # Assuming you have this available

#     @torch.inference_mode()
#     def prepare_inputs(self, sample) -> Dict[str, Any]:
#         """Prepare inputs for a single sample."""
#         image_tensor = None
#         t0 = time.perf_counter()
#         img = Image.open(sample.image_path).convert("RGB") if sample.image_path else None
...
#     @torch.inference_mode()
#     def generate(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
#         from scripts.timer import StageTimer # Import locally if needed
#         timer = StageTimer(use_cuda=(self.device().type == "cuda"))
#         timer.start("end2end")
...
class LlavaSparseZipModel(LlavaModel):
    """
    LLaVA + VisionZip + SparseZip (+ Text-Aware scoring)
    """
    def __init__(
        self,
        model_path: str,
        ...
        # ------------------------------------------------------------------
        # Patch encode_images: 先 vision_tower，再 mm_projector（保持 SparseZip 友好）
        # ------------------------------------------------------------------
        ...
        # VisionZip 里对 CLIP 的 monkey-patch（注意要在 load 模型前）
        LlavaVisionZipModel._apply_visionzip_monkey_patches()

        # ------------------------------------------------------------------
        # 先按原来的方式加载 LLaVA 基础模型
        # ------------------------------------------------------------------
        super().__init__(
            model_path=model_path,
            temperature=temperature,
        ...
        # ------------------------------------------------------------------
        # VisionZip 包装（dominant + contextual）
        # ------------------------------------------------------------------
        from visionzip import visionzip as _visionzip
        self.model = _visionzip(self.model, dominant=dominant, contextual=contextual)
        self.model.eval()
        ...
        # ==============================
        # [SparseZip TextAware] 1) 挂 text->key 映射到 CLIPVisionTower 类
        # ==============================
        import torch
        import torch.nn as nn
        from llava.model.multimodal_encoder.clip_encoder import CLIPVisionTower
        ...
        # debug 当前 vt
        print("[SparseZip Debug] vt in __init__:", type(vt), "id=", id(vt))

        core_lm = getattr(self.model, "model", None)
        if core_lm is None:
            core_lm = self.model  # 兜底

        if hasattr(core_lm, "embed_tokens"):
            d_llm = core_lm.embed_tokens.embedding_dim
        else:
            d_llm = getattr(self.model.config, "hidden_size", None)
            if d_llm is None:
                raise RuntimeError(
                    "[SparseZip] Cannot infer d_llm: neither embed_tokens nor config.hidden_size is available."
                )

        vt_inst = vt[0] if isinstance(vt, list) else vt
        clip_cfg = vt_inst.vision_tower.config

        # 对 CLIP 来说，metric 的维度通常是 head_dim = hidden_size // num_attention_heads
        if hasattr(clip_cfg, "num_attention_heads") and clip_cfg.num_attention_heads > 0:
            d_clip_key = clip_cfg.hidden_size // clip_cfg.num_attention_heads
        else:
            d_clip_key = clip_cfg.hidden_size  # 兜底：如果以后结构变了，再退回 hidden_size

        # 挂在“类”上，保证所有实例都能访问
        if not hasattr(CLIPVisionTower, "_text2key"):
            CLIPVisionTower._text2key = nn.Linear(d_llm, d_clip_key, bias=False).to(vt_inst.device)
            CLIPVisionTower._text2key.requires_grad_(False)
            print(
                "[SparseZip TextAware] attached class-level _text2key to CLIPVisionTower, "
                f"proj: {d_llm} -> {d_clip_key}"
            )
        else:
            print("[SparseZip TextAware] class-level _text2key already exists on CLIPVisionTower")

        # ==============================
        # [SparseZip TextAware] 2) monkey-patch LlavaLlamaForCausalLM.prepare_inputs_labels_for_multimodal
        # ==============================
        from llava.model.language_model.llava_llama import LlavaLlamaForCausalLM

        if not getattr(LlavaLlamaForCausalLM, "_sparsezip_textaware_patched", False):
            _orig_prepare = LlavaLlamaForCausalLM.prepare_inputs_labels_for_multimodal

            def _prepare_inputs_labels_for_multimodal_textaware(
                self,
                input_ids,
                attention_mask,
                past_key_values,
                ...
                **kwargs,
                ...
                if (
                    ...
                    try:
                        print("[SparseZip TextAware] setting _text_query in LlavaLlamaForCausalLM.prepare_inputs...")
                        text_embeds = self.get_model().embed_tokens(input_ids)  # [B,T,d_llm]
                        ...
                        masked = text_embeds * text_mask.unsqueeze(-1)
                        denom = text_mask.sum(dim=1, keepdim=True).clamp_min(1)
                        query_vec = masked.sum(dim=1) / denom  # [B,d_llm]

                        vt_local = vision_tower
                        if isinstance(vt_local, list):
                            vt_local = vt_local[0]

                        vt_local._text_query = query_vec.detach().to(vt_local.device)
                        print("[SparseZip TextAware] _text_query set, shape:", query_vec.shape)
                    except Exception as e:
                        print("[SparseZip TextAware] failed to set _text_query in patched prepare:", repr(e))
            ...
            LlavaLlamaForCausalLM.prepare_inputs_labels_for_multimodal = (
                _prepare_inputs_labels_for_multimodal_textaware
            )
            LlavaLlamaForCausalLM._sparsezip_textaware_patched = True
            print("[SparseZip TextAware] patched LlavaLlamaForCausalLM.prepare_inputs_labels_for_multimodal")

        # ------------------------------------------------------------------
        # 下面是原来的 SparseZip scoring + compressor 配置
        # ------------------------------------------------------------------
        if skip_hybrid_attn:
        ...
        # 把压缩器挂到 vision_tower 上，供 CLIPVisionTower_SparseZip_EXP.forward 使用
        vt_inst._vz_comp = vz_comp
        self._sparsezip_cfg = comp_cfg

        # ------------------------------------------------------------------
        # Patch CLIPVisionTower.forward -> 我们自己的 SparseZip 版本
        # ------------------------------------------------------------------
    ...
    def prepare_inputs(self, sample: Sample) -> Dict[str, Any]:
        ...
        return inputs
        print(
            "[SparseZip] VisionZipCompressor attached. "
            f"ctx={ctx_num}, k_range=[{k_min_eff},{k_max_eff}], "
            f"alphas=({alphas.attn},{alphas.entropy},{alphas.mutual}), "
            f"dynamic_k={use_dynamic_k}, skip_ctx_merge={skip_ctx_merge}, "
            f"skip_hybrid_attn={skip_hybrid_attn}, skip_dynamic_k={skip_dynamic_k}"
    ...
    def generate(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        """Generates the response and collects timing metrics.
        在原有基础上增加：
          - 在调用 self.model.generate 之前，根据当前 batch 的 input_ids 计算 text_query
          - 写入 vision_tower._text_query，供 CLIPVisionTower_SparseZip_EXP 使用
        """
        timer = StageTimer(use_cuda=(self.device().type == "cuda"))

        input_ids = inputs["input_ids"]   # [B, T]
        images    = inputs["images"]      # [B, 3, H, W] or None

        # ============================================================
        # [SparseZip TextAware] 用当前 batch 的 input_ids 构造 text query
        # ============================================================
        if images is not None and input_ids is not None and input_ids.shape[1] > 1:
            try:
                # 1) 拿到 LLM 主体（有 embed_tokens 的那个）
                core_lm = getattr(self.model, "model", self.model)
                if not hasattr(core_lm, "embed_tokens"):
                    if hasattr(self.model, "get_model"):
                        core_lm = self.model.get_model()
                if not hasattr(core_lm, "embed_tokens"):
                    raise RuntimeError(
                        "[SparseZip TextAware] core_lm has no embed_tokens, cannot build text query."
                    )

                embeddings = core_lm.embed_tokens       # nn.Embedding
                vocab_size = embeddings.num_embeddings  # 词表大小

                # 2) 构造 safe_ids：把 IMAGE_TOKEN_INDEX 等潜在越界 id 换成合法的 pad / eos / 0
                safe_ids = input_ids.clone()

                # 标记图像 token（后面做 mask 用）
                image_mask = (safe_ids == self.IMAGE_TOKEN_INDEX)

                # 选择一个一定在 [0, vocab_size-1] 内的安全 id 作为“占位”
                ...
                # 把 IMAGE_TOKEN_INDEX 位置替换成安全的 pad_id，避免 embed_tokens 越界
                safe_ids[image_mask] = pad_id

                # 再保险一点：clip 一下所有 id，防止有其他脏 index
                safe_ids = safe_ids.clamp(min=0, max=vocab_size - 1)

                # 3) 真正做 embedding：现在 safe_ids 肯定全在合法范围内
                text_embeds = embeddings(safe_ids)   # [B, T, d_llm]

                # 4) 构造聚合 mask：
                #    - 不要 pad id
                #    - 不要原始 IMAGE_TOKEN_INDEX 对应的位置（只聚合纯文本 token）
                mask = torch.ones_like(safe_ids, dtype=torch.bool, device=safe_ids.device)

                # 去 pad
                if pad_id is not None and 0 <= pad_id < vocab_size:
                    mask = mask & (safe_ids != pad_id)
                # 去图像 token
                mask = mask & (~image_mask)

                # 防止极端情况下 mask 全 0
                if mask.sum() == 0:
                    # 至少保证非 image 的 token 参与
                    mask = ~image_mask
                    if mask.sum() == 0:
                        # 连非 image 的 token 都没有，就直接用所有 token
                        mask = torch.ones_like(safe_ids, dtype=torch.bool, device=safe_ids.device)

                # 5) masked 平均得到 query_vec: [B, d_llm]
                masked_embeds = text_embeds * mask.unsqueeze(-1)            # [B,T,d]
                denom = mask.sum(dim=1, keepdim=True).clamp_min(1)          # [B,1]
                query_vec = masked_embeds.sum(dim=1) / denom                # [B,d_llm]

                # 6) 写到 vision_tower 上，让 CLIPSparseZip 用来算 cross_attn
                vt = self.model.get_vision_tower()
                if isinstance(vt, list):
                    vt = vt[0]
                vt._text_query = query_vec.detach().to(vt.device)

                # print(
                #     "[SparseZip TextAware] generate(): _text_query set on vision_tower, shape:",
                #     query_vec.shape,
                # )
            except Exception as e:
                print("[SparseZip TextAware] generate() failed to set _text_query:", repr(e))

        # ============================================================
        # 下面保持你原来的 coarse timing + generate 逻辑不变
        # ============================================================
        timer.start("end2end")
        timer.start("decode")  # Treating the entire generation as the decode phase for coarse timing
