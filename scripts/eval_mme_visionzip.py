#!/usr/bin/env python3
"""
MME Evaluation Script for VisionZip
Evaluates VisionZip model on MME benchmark (2374 questions across 14 categories)
"""
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'models', 'LLaVA'))
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'models', 'VisionZip'))
...
def eval_mme_visionzip(args):
    """Run MME evaluation with VisionZip token selection"""
    
    print("=" * 80)
    print("MME Evaluation - VisionZip")
    print("=" * 80)
    ...
    # Load model ONCE
    print("\n2. Loading VisionZip model...")
    print(f"   Model: {args.model_path}")
    print(f"   Dominant tokens: {args.dominant_tokens}")
    print(f"   Contextual tokens: {args.contextual_tokens}")
    print(f"   Total tokens: {args.dominant_tokens + args.contextual_tokens}")
    ...
    # Set VisionZip parameters
    model.config.mm_vision_select_feature = "patch"
    model.config.image_aspect_ratio = "pad"
    model.config.pruning_method = "visionzip"
    model.config.dominant_ratio = args.dominant_tokens
    model.config.contextual_ratio = args.contextual_tokens
    
    # Apply VisionZip (clip_layer is handled internally, defaults to layer 22)
    visionzip(model, dominant=args.dominant_tokens, contextual=args.contextual_tokens)
    
    print("✓ VisionZip applied to model")
    
    # Use cuda:0 for inputs (model will handle CPU offloading internally)
    ...
    with torch.no_grad():
        for idx, q in enumerate(tqdm(questions, desc="Evaluating")):
            try:
                ...
                # Generate - match POPE evaluation exactly
                ...
                # VisionZip may modify tokenization, causing invalid token IDs
                # Use prompt string directly and slice by token length
    ...
    print("\nTo compare with SparseVLM, run:")
    print("  python scripts/compare_mme_results.py")
    print("=" * 80)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate VisionZip on MME benchmark")
    ...
    # VisionZip arguments
    parser.add_argument("--dominant-tokens", type=int, default=54,
                       help="Number of dominant tokens")
    parser.add_argument("--contextual-tokens", type=int, default=10,
                       help="Number of contextual tokens")
    parser.add_argument("--clip-layer", type=int, default=22,
                       help="CLIP layer for attention extraction")
    ...
    parser.add_argument("--output-file", type=str,
                       default="/u/m/a/maheenabooba/cs769/models/LLaVA/playground/eval/MME/answers/visionzip-v1.5-7b.jsonl",
                       help="Output file for answers")
    parser.add_argument("--experiment-name", type=str, default="visionzip-v1.5-7b",
                       help="Name for this experiment")
    
    args = parser.parse_args()
    eval_mme_visionzip(args)
