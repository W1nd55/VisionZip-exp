...
def main():
    ...
    if args.max_samples and args.max_samples > 0:
        ...
        # Randomly sample
        if len(all_questions) > args.max_samples:
            sampled_questions = random.sample(all_questions, args.max_samples)
            print(f"   Selected {args.max_samples} random samples out of {len(all_questions)} total")
        else:
            sampled_questions = all_questions
            print(f"   Using all {len(all_questions)} questions (less than requested)")
            ...
            for file in os.listdir(annotation_dir):
                if file.startswith('coco_pope_') and file.endswith('.json'):
                    ...
                    if cur_answers:
                        ...
                        eval_pope(cur_answers, label_file)
                        print("   " + "=" * 50)
